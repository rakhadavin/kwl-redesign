...
export default function LearnedPageQuiz() {
  ...
  const { data } = useGetObjects(`/api/learned/quiz/${id_topic}`, "know");
  ...
  return (
    <main className="py-5 text-center bg-[url('/bg1.png')] bg-cover min-h-screen">
      <Breadcrumb items={[{ label: "Beranda", href: "/courses" }, { label: course?.data?.short_name ?? "...", href: `/courses/${id_course}` }, { label: topic?.data?.name ?? "..." }, { label: "Learned" }, { label: "Quiz" }]} />
