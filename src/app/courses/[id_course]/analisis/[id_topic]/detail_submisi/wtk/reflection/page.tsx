...
const DetailSubmissionWtkEssayPage = () => {
  ...
  const { data } = useGetAuth(
    ...
    "detail analisis wtk", true
  );

  const { data: wordCloud, status } = useGetAuth(
    `/api/analysis/wordcloud/wtk/${params.id_topic}`,
    "wordcloud wtk"
  );

  return (
    <main className="py-5">
      <Breadcrumb items={[{ label: "Beranda", href: "/courses" }, { label: data?.["course_data"]?.["full_name"] ?? "...", href: `/courses/${params.id_course}` }, { label: "Analisis", href: `/courses/${params.id_course}/analisis` }, { label: data?.["name"] ?? "...", href: `/courses/${params.id_course}/analisis/${params.id_topic}` }, { label: "WTK" }, { label: "Refleksi" }]} variant="light" />
...
export default DetailSubmissionWtkEssayPage;
