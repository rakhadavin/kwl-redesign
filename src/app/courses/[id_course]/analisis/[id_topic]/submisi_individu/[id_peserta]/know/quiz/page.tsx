...
export default function DetailAnswerKnowQuizPage() {

   
    const { id_topic, id_peserta, id_course } = useParams();
    const { data: answer } = useGetAuth(`/api/analysis/student-answer-detail/know/${id_topic}/${id_peserta}`, "checkbox");
    ...
    return (
      <main className="py-5 text-center bg-[url('/bg1.png')] bg-cover min-h-screen">
        <Breadcrumb items={[{ label: "Beranda", href: "/courses" }, { label: answer?.course ?? "...", href: `/courses/${id_course}` }, { label: "Analisis", href: `/courses/${id_course}/analisis` }, { label: answer?.topic ?? "...", href: `/courses/${id_course}/analisis/${id_topic}` }, { label: "Know" }, { label: "Quiz" }]} />
