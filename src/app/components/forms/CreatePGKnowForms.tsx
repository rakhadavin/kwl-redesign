...
const CreatePGKnowForms = () => {
  ...
  const { mutate } = usePostAuth("/api/know/quiz", "lecturer");
  ...
  const addSoal = () => {
    append({ ...EMPTY_SOAL });
    setCurrentIdx(fields.length); // fields.length = new index after append
  };
  ...
  return (
    <Modal
      ...
      content={content}
      isOpen={createPGKnowForms.isOpen}
      close={createPGKnowForms.close}
    />
  );
};

export default CreatePGKnowForms;