...
export default function LearnedPageQuiz() {
  ...
  const { data, isLoading } = useGetObjects(`/api/learned/quiz/${id_topic}`, "know");
  ...
  return (
    <main className="py-5 text-center bg-[url('/bg1.png')] bg-scroll bg-cover min-h-screen">
            ...
            >
              <div className="w-[90%] md:w-[80%] lg:w-[800px] relative bg-white rounded-lg shadow flex flex-col max-h-[88vh]">
                ...
                <div className="shrink-0 px-8 py-5 flex gap-4 justify-center border-t border-gray-100">
                  ...
                  {(() => {
                    const isAnswered = content.options.some((o: any) => checkedItems.includes(o.id));
                    const isDisabled = !isAnswered || isPending;
                    return (
                      <button
                        onClick={increment}
                        disabled={isDisabled}
                        className={`text-xs text-white font-bold w-[120px] py-2 shadow text-center rounded flex items-center justify-center gap-1 ${isDisabled ? "bg-green-300 cursor-not-allowed" : "bg-green-500"}`}
                      >
      ...
      <Modal
        open={Backopen}
        description="Pergi ke halaman class transition?"
        closeText="kembali"
        nextText="konfirmasi"
      ...
      <ConfirmationModal
        ...
        url={`/peserta/courses/${id_course}/${id_topic}/poin`}
      />
