...
const DetailSubmissionKnowEssayPage = () => {
  ...
  const { data } = useGetAuth(
    ...
    "detail analiisa know"
  );

  const { data: wordCloud, status } = useGetAuth(
    `/api/analysis/wordcloud/know/${params.id_topic}`,
    "wordcloud know"
  );
    ...
    <main className="py-5">
      ...
      {status === "success" ? (
        <div>
          ...
          <Link href={`${newPath}/submisi_individu/${session?.userinfo.role_pk}/know/reflection`}>
...
export default DetailSubmissionKnowEssayPage;
